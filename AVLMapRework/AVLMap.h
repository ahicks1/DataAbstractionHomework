...
class AVLMap {
	...
	void rrotate(Node* n){
		Node *m = n->left;
		if(!m) {
			cout << "ILLEGAL RIGHT ROTATE!" << endl;
			return;
		}
		if (n->parent ==nullptr) root= m;
	...
	void restore_balance(Node* n) {
		//Ensure the height is properly set first?
		seth(n);

		//Get stuff setup for the main loop
		Node * rover = n;
    int balance = 0;
		...
		while(rover != nullptr) {
			...
			if(balance > 1) { //Requires at least a right rotate
				...
				if(get_height(rover->left->left) >= get_height(rover->left->right)) { //Requires only one rotate
					//cout << "single right" << endl;
					rrotate(rover);
				} else { //Requires two rotations
				...
				if(get_height(rover->right->right) >= get_height(rover->right->left)) { //Requires only one rotate
...
public:
	...
	void verifyBalance() {
		cout << verifyBalanceRec(root) << endl;
	}

private:

	bool verifyBalanceRec(Node *n) {
		if(!n) return true;
		auto balance = get_height(n->left)-get_height(n->right);
		auto isBalanced = balance*balance <= 1;
		if (isBalanced){
			return verifyBalanceRec(n->left) && verifyBalanceRec(n->right);
		} else {
			cout << "Balance factor off! " << balance << endl;
			return false;
		}
	}
	...
	void printTreeRecur(Node *n) {
		if(n!=nullptr) {
			...
			std::cout << n->height << "-" ;//<< n->data.second << " ";
			printTreeRecur(n->right);
			std::cout << ")";
		}
	}
	void removeNode(Node *rover) {
		Node* changed = rover->parent;
